...
export const ShiftManagement = () => {
  ...
  const avgDuration = shifts.reduce((acc, shift) => {
  ...
  const exportToPDF = () => {
    ...
    doc.text(`Date Range: ${formatDate(currentDate)}`, 105, 28, { align: 'center' });
    ...
    doc.text(`On Time Shifts: ${onTimeShifts} (${totalShifts > 0 ? Math.round((onTimeShifts / totalShifts) * 100) : 0}%)`, 14, 56);
    doc.text(`Late Shifts: ${lateShifts} (${totalShifts > 0 ? Math.round((lateShifts / totalShifts) * 100) : 0}%)`, 14, 64);
